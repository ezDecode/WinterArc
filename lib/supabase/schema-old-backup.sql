...
-- Add constraints (optional timezone validation)
-- Note: Timezone validation removed as it's too restrictive for some valid timezones
...
-- Create base indexes for daily_entries
...
-- Note: Complex validation constraints removed
-- Validation is handled at the application level for better flexibility
-- The default values ensure correct structure on insert
...
-- Create checkpoint_notes table if it doesn't exist
...
BEGIN
  ...
  -- profiles table uses 'id' as the user identifier
  -- other tables use 'user_id'
  IF TG_TABLE_NAME = 'profiles' THEN
    audit_user_id := COALESCE(NEW.id, OLD.id);
  ELSE
    -- Try to get user_id from the record, fall back to current user
    BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        audit_user_id := public.get_user_id();
    END;
  END IF;
