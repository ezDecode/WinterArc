...
export const DailyScoreDisplay = memo(function DailyScoreDisplay({
  ...
  return (
    <section 
      className="bg-surface border border-border rounded-xl p-4 sm:p-6 animate-in hover:border-border/80 transition-all duration-200 shadow-sm" 
      aria-labelledby="daily-score-heading"
      role="region"
    >
      <h3 
        id="daily-score-heading"
        className="text-sm font-medium text-text-secondary mb-4 flex items-center gap-2"
      >
      ...
      {/* Progress Bar */}
      <div className="mb-4 sm:mb-6" role="progressbar" aria-valuenow={score} aria-valuemin={0} aria-valuemax={DAILY_MAX_SCORE} aria-label={`Progress: ${score} out of ${DAILY_MAX_SCORE} points completed`}>
      ...
      <div className="space-y-2.5" role="list" aria-label="Score breakdown by category">
        <div className="text-xs font-medium text-text-tertiary mb-2 uppercase tracking-wide">Breakdown</div>
        <div className="flex justify-between text-xs sm:text-sm py-1.5 px-2 rounded-lg bg-background/50" role="listitem">
      ...
      {isComplete && (
        <div 
          className="mt-4 sm:mt-6 p-3 sm:p-4 bg-success/10 border border-success/20 rounded-xl animate-in shadow-sm"
          role="alert"
          aria-live="polite"
        >
